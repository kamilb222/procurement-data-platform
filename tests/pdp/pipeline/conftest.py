...
(init + load + staging + transform + marts) against the synthetic fixture,
so the staging, transform, and marts test modules share a single container.
Uses testcontainers, so nothing here touches the docker-compose dev database
or requires the real dataset.
"""
...
def pipeline_conn() -> Iterator[psycopg.Connection]:
    """Run init + load + staging + transform + marts against the fixture in a fresh container."""
