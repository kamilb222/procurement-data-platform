"""Integration test for the staging layer, driven by the shared pipeline_conn
fixture (init + load + staging + transform against the synthetic fixture in a
throwaway Postgres container; see conftest.py)."""
...
def test_reconciliation_holds(pipeline_conn: psycopg.Connection) -> None:
    (raw_count,) = _one(pipeline_conn, "SELECT count(*) FROM raw.purchase_orders")
    (staging_count,) = _one(pipeline_conn, "SELECT count(*) FROM staging.purchase_orders")
    (rejected_count,) = _one(pipeline_conn, "SELECT count(*) FROM staging.rejected_rows")
...
def test_rejected_rows_have_the_expected_reasons(pipeline_conn: psycopg.Connection) -> None:
...
def test_remove_amerisource_column_is_dropped(pipeline_conn: psycopg.Connection) -> None:
    rows = _one(
        pipeline_conn,
        """
...
def test_accounting_negative_price_is_parsed(pipeline_conn: psycopg.Connection) -> None:
    (unit_price,) = _one(
        pipeline_conn,
...
def test_mojibake_is_repaired(pipeline_conn: psycopg.Connection) -> None:
    (item_name,) = _one(
        pipeline_conn,
...
def test_location_is_split_into_zip_lat_lon(pipeline_conn: psycopg.Connection) -> None:
    zip_, lat, lon = _one(
        pipeline_conn,
...
def test_duplicate_rows_are_not_deduplicated_in_staging(pipeline_conn: psycopg.Connection) -> None:
    (count,) = _one(
        pipeline_conn,
